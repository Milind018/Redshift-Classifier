...
numCores = detectCores() -1  # Leave two cores out so you can still use your machine or if you use the cluster then use all the cores
cluster=parallel::makeCluster(numCores,outfile="")
parallel::clusterEvalQ(cluster, .libPaths(c("~/Rlibs", .libPaths())))
parallel::clusterEvalQ(cluster, library(SuperLearner), library(arm))
