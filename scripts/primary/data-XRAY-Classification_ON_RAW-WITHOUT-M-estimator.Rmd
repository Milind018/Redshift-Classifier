...
.libPaths("~/Rlibs")# Comment this out if using local computer
...
numCores = detectCores() - 12  # Leave two cores out so you can still use your machine or if you use the cluster then use all the cores
cluster=parallel::makeCluster(numCores,outfile="")
parallel::clusterEvalQ(cluster, .libPaths(c("~/Rlibs", .libPaths()))) # Comment this out if using local computer
parallel::clusterEvalQ(cluster, library(SuperLearner) )
parallel::clusterEvalQ(cluster, library(arm) )
...
#set.seed(12345)
system.time({
  numCores = detectCores() - 12  # Leaving two cores out so you can still use your machine or if you use the cluster then use all the cores
