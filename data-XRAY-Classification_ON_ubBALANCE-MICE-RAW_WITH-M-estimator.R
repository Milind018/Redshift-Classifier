...
PLOTaddr <- "CURRENT_ANALYSIS/Graphics/XRAY/BALANCING/MICE_WITH_M-estimator/"
...
if(!dir.exists('CURRENT_ANALYSIS/Graphics/XRAY/BALANCING')){dir.create('CURRENT_ANALYSIS/Graphics/XRAY/BALANCING')}
if(!dir.exists('CURRENT_ANALYSIS/Graphics/XRAY/BALANCING/MICE_WITH_M-estimator')){dir.create('CURRENT_ANALYSIS/Graphics/XRAY/BALANCING/MICE_WITH_M-estimator')}
...
#GRBData_raw_data_w_errorbar <- read.csv(file = "/home/jarvis-astro/PhD/NAOJ_Maria-Giovanna-Dainotti/GRBmachinelearning/Dankworth_SULI_2021/CURRENT_ANALYSIS/Upsampling/XRAY/M-estimator/MICE/FULL_ubsampled_correctheader_XRAY_DATA_with_errorbar_after_MICE_after_M-estimator.csv", header = TRUE)
## Loading the raw data without errorbars and MICE imputed. ##
run_locally = F
if(run_locally){
GRBData_raw_data_w_errorbar <- read.csv(file = "CURRENT_ANALYSIS/Upsampling/XRAY/MICE/TEST/FULL_ubsampled_correctheader_XRAY_DATA_with_errorbar_after_MICE_ON_RAW_with_M-estimator.csv", header = TRUE)
cutoff=3.5} else {
...
n_test <- round(nrow(GRBData10) * 0.2)  # 20% of the data for Test Set
n_training <- nrow(GRBData10) - n_test  # Remaining data for Training Set

# Split the data into Training and Test sets using specific row indices
TestData <- GRBData10[1:n_test, ]
TrainingData <- GRBData10[(n_test + 1):nrow(GRBData10), ]
str(TrainingData)
dim(TrainingData)
str(TestData)
dim(TestData)
...
#set.seed(12345)
system.time({
  numCores = detectCores() - 1  # Leaving two cores out so you can still use your machine or if you use the cluster then use all the cores
  ...
  SL_CVmodel<-foreach(i = 1:loop, .packages=c("SuperLearner"), .export = libs_SL)
    ...
    SL_model<-CV.SuperLearner(Y = Training_len,X =TrainingData[,LassoVars], cvControl= list(V = 10, stratifyCV=T), family = binomial(), method = "method.AUC", SL.library = libs_SL, control = list(saveFitLibrary=T)) # 10-fold CV with ALL variables
...
libs_SL_CV = c("SL.randomForest", "SL.ranger","SL.svm")
loop <- 100
...
rect_x <- 0.52
rect_y_bottom <- legend_y - 0.044 * (length(libs_SL_CV) + 2)
